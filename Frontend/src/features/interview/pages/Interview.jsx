...
// ── Sub-components ────────────────────────────────────────────────────────────
const QuestionCard = ({ text, index }) => (
    <div className='q-card'>
        <div className='q-card__header'>
            <span className='q-card__index'>Q{index + 1}</span>
            <p className='q-card__question'>{text}</p>
        </div>
    </div>
)
    ...
    <div className='plan-list'>
        {items.map((item, i) => (
            <div key={i} className='plan-item'>
                <span className='plan-item__index'>{i + 1}</span>
                <p className='plan-item__text'>{item}</p>
            </div>
        ))}
    </div>
)
...
const Interview = () => {
    ...
    // ✅ SAFE REPORT (prevents undefined crashes)
    const safeReport = {
                    ...
                    {activeNav === 'roadmap' && (
                        <section>
                            <div className='content-header'>
                                ...
                                <span className='content-header__count'>
                                    {safeReport.preparationPlan.length} items
                                </span>
                            </div>

                            <PlanList items={safeReport.preparationPlan} />
                        </section>
                    )}
